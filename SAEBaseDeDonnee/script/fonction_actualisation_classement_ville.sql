...
CREATE OR REPLACE FUNCTION actualisation_cla_ville(id_vil ville.id_ville%type, pts_a_ajouter_ou_a_retirer INTEGER)
returns void as $$

DECLARE

    /* Une variable contenant les pts actuels du joueurs, permettant l'actualisation */
    pts classement_villes.ville_pts%type;

BEGIN

    /* On selectionne ses pts, on y ajoute la nouvelle valeur (celle en paramètres) */
    SELECT ville_pts into pts  FROM classement_villes  where id_ville=id_vil ;
    pts=pts+pts_a_ajouter_ou_a_retirer;
    UPDATE classement_villes
    SET ville_pts = pts
    where id_ville=id_vil;
...
BEGIN
    ...
    PERFORM actualisation_cla_ville(id_ville_jou_gagnant,pts_jou_gagnant);
    PERFORM actualisation_cla_ville(id_ville_jou_vaincue,pts_jou_vaincue);
    return NEW;
...
CREATE OR REPLACE trigger  actu_class_vil
